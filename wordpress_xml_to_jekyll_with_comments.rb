...
class WordPressXMLToJekyllWithComments
  ...
  def migrate_all
    puts "Starting WordPress XML migration with comments from #{@xml_file_path}"
    ...
    puts "Imported: #{@comment_count} comments"
  ...
  def process_item(item)
    begin
      ...
      year_path = File.join(@jekyll_posts_path, year)
      FileUtils.mkdir_p(year_path) unless Dir.exist?(year_path)
      
      filename = "#{date.strftime('%Y-%m-%d')}-#{slug}.md"
      filepath = File.join(year_path, filename)
      ...
      # Generate Jekyll front matter with comments
      front_matter = generate_front_matter(title, date, categories, tags, excerpt, comments)
      ...
      @migrated_count += 1
      comment_note = comments.any? ? " (#{comments.length} comments)" : ""
      puts "Migrated: #{year}/#{filename}#{comment_note}"
  ...
  def extract_comments(item)
    comments = []
    item.xpath('.//wp:comment').each do |comment_node|
      begin
        comment_type = extract_text(comment_node, 'wp:comment_type')
        comment_approved = extract_text(comment_node, 'wp:comment_approved')
        
        # Only include approved comments of type 'comment' (not pingbacks/trackbacks)
        next unless comment_type == 'comment' && comment_approved == '1'
        
        comment = {
          'id' => extract_text(comment_node, 'wp:comment_id'),
          'author' => extract_text(comment_node, 'wp:comment_author'),
          'author_email' => extract_text(comment_node, 'wp:comment_author_email'),
          'author_url' => extract_text(comment_node, 'wp:comment_author_url'),
          'date' => extract_text(comment_node, 'wp:comment_date'),
          'content' => extract_text(comment_node, 'wp:comment_content'),
          'parent' => extract_text(comment_node, 'wp:comment_parent')
        }
        
        # Clean up empty fields
        comment.delete_if { |k, v| v.nil? || v.strip.empty? }
        
        comments << comment
      rescue => e
        puts "Warning: Could not process comment: #{e.message}"
      end
    end
    
    # Sort comments by date
    comments.sort_by! { |c| DateTime.parse(c['date']) rescue DateTime.new(1970, 1, 1) }
  ...
  def generate_front_matter(title, date, categories, tags, excerpt, comments)
    front_matter = "---\n"
    ...
    if comments.any?
      front_matter += "comments:\n"
      comments.each do |comment|
        front_matter += "  - id: #{comment['id']}\n"
        front_matter += "    author: \"#{comment['author'].gsub('"', '\"')}\"\n" if comment['author']
        front_matter += "    author_email: \"#{comment['author_email']}\"\n" if comment['author_email']
        front_matter += "    author_url: \"#{comment['author_url']}\"\n" if comment['author_url']
        front_matter += "    date: \"#{comment['date']}\"\n"
        front_matter += "    content: |\n"
        # Indent comment content properly for YAML
        comment_lines = comment['content'].split("\n")
        comment_lines.each do |line|
          front_matter += "      #{line}\n"
        end
        front_matter += "    parent: #{comment['parent']}\n" if comment['parent'] && comment['parent'] != '0'
      end
    end
...
if __FILE__ == $0
  ...
  migrator = WordPressXMLToJekyllWithComments.new(xml_path, jekyll_path)
